function UpdateData(connection){
  return function (req, res) {
    ...
    if (typeof id == 'undefined' || !id.match(/^[0-9]+$/)){
      ...
      recursiveSQLUpdate(tableName, connection, data, 0, function(){
...
function recursiveSQLUpdate(tableName, connection, bodyArray, nextIndex, callback){
  if (bodyArray[nextIndex] !== undefined) {
    ...
    connection.query("UPDATE " + tableName + " SET ? WHERE ?", [varsToUpdate, {'id': currentID}], function(err, result){
      ...
      recursiveSQLUpdate(tableName, connection, bodyArray, nextIndex+1, callback);
    });
  }else{
    callback();
  }
